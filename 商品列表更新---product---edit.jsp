...
<HTML>
	<HEAD>
		...
		/*
			//页面加载完毕之后，再去确定那个option被选中
			window.onload = function(){
				//1.获得当前回显的product的cid
				var cid = "${product.cid}";
				
				//2.获得所有的<select name="cid">下的option
				var options = document.getElementById("cid").getElementsByTagName("option");
				
				//alert(options.length);//查看一下获取是否成功
			
				//3.比较每一个option的value与cid进行比较，若是相同，代表被选中
	...
	<body>
		...
		<form id="userAction_save_do" name="Form1" action="${pageContext.request.contextPath}/adminUpdateProduct" method="post" ">
			...
			<input type="hidden" name="pid" value="${product.pid}"><!-- 隐藏域 :用户不需要看到，但是后台还需要，则使用隐藏域传-->
